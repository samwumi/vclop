...
// The status transitions a Loan Officer/Compliance user can move a customer through from this screen.
const NEXT_STATUS_OPTIONS: CustomerStatus[] = ['KYC_PENDING', 'KYC_VERIFIED', 'ELIGIBLE', 'INELIGIBLE', 'BLACKLISTED'];
...
function CustomerOverviewTab({
  customer: c, onStatusChange, canEdit,
}: { customer: Customer; onStatusChange: (s: CustomerStatus) => void; canEdit: boolean }) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
      ...
      <div className="card">
        ...
        <div className="card-body">
          <p className="text-xs text-gray-500 mb-3">Move this customer through the KYC/eligibility lifecycle</p>
          <div className="flex flex-wrap gap-2">
            {NEXT_STATUS_OPTIONS.map((status) => (
              <button
                key={status}
                disabled={!canEdit || c.status === status}
