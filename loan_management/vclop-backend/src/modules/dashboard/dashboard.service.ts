...
export class DashboardService {
  ...
  async getOperationalSummary(user: RequestUser) {
    ...
    const canDisburse = isAdministrator || user.permissions.has('loan_applications:disburse');
    ...
    const ownedApplications = !canReview && !canDisburse && !canManage;
    const applicationScope = ownedApplications ? { submittedById: user.id } : {};
    const [myTasks, applications, complianceQueue, approvedLoans, collectionCases, transportRequests, overdueInstallments] = await Promise.all([
      ...
      canReview ? this.prisma.loanApplication.count({ where: { deletedAt: null, status: { in: ['COMPLIANCE_REVIEW', 'AWAITING_INFORMATION'] } } }) : 0,
      ...
      canReview || canManage ? this.prisma.transportRequest.count({ where: { status: { in: ['PENDING', 'OPERATIONS_REVIEW'] } } }) : 0,
    ...
    return { role, myTasks, applications, complianceQueue, approvedLoans, collectionCases, transportRequests, overdueInstallments };
  }
      ...
      data: {
        ...
        items: {
          create: dto.items.map((item) => ({
            ...
            width: item.width,
            height: item.height,
            config: item.config ? (item.config as object) : undefined,
          })),
        },
      },
      include: {
