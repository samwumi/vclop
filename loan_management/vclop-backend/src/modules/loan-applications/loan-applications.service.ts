...
export class LoanApplicationsService {
  ...
  async review(applicationId: string, dto: ReviewLoanApplicationDto, actor: RequestUser): Promise<unknown> {
    ...
    if (dto.decision === ReviewDecision.APPROVED) {
      newStatus = application.status === LoanApplicationStatus.COMPLIANCE_REVIEW 
      ...
      // REQUEST_INFORMATION
      newStatus = LoanApplicationStatus.AWAITING_INFORMATION;
    }
    ...
    await this.prisma.loanApplication.update({
      ...
      data: {
        ...
        rejectionReason: (dto.decision === ReviewDecision.REJECTED || dto.decision === ReviewDecision.REQUEST_INFORMATION) ? dto.rejectionReason : null,
      },
    });

    const actionLabel = dto.decision === ReviewDecision.APPROVED 
      ? 'Approved' 
      : dto.decision === ReviewDecision.REJECTED 
        ? 'Rejected' 
        : 'Requested more information for';

    this.emitAudit(AuditAction.UPDATE, actorId, applicationId, `${actionLabel} ${application.applicationNumber}`);
    this.events.emit('loan_application.reviewed', { applicationId, decision: dto.decision, actorId });
    return this.findOne(applicationId);
  }

  /** AWAITING_INFORMATION -> COMPLIANCE_REVIEW. Allows resubmission after providing requested information. */
  async resubmitWithInformation(applicationId: string, actorId: string): Promise<unknown> {
    const application = await this.prisma.loanApplication.findFirst({
      where: { id: applicationId, deletedAt: null },
      include: { loanProduct: { include: { documentRequirements: true } }, guarantors: true, collaterals: true },
    });
    ...
    if (application.status !== LoanApplicationStatus.AWAITING_INFORMATION) {
      throw new BusinessException(`Only applications AWAITING_INFORMATION can be resubmitted (currently ${application.status})`);
    }

    // Re-check all requirements (same as initial submission)
    if (application.loanProduct.requiresGuarantor && application.guarantors.length === 0) {
      throw new BusinessException(`${application.loanProduct.name} requires at least one guarantor`);
    }
    if (application.loanProduct.requiresCollateral && application.collaterals.length === 0) {
    ...
    this.emitAudit(AuditAction.UPDATE, actorId, applicationId, `Resubmitted ${application.applicationNumber} with requested information`);
    this.events.emit('loan_application.resubmitted', { applicationId, actorId });
    return this.findOne(applicationId);
  }
