...
export class AuthService {
  ...
  async requestPasswordChangeOTP(userId: string): Promise<{ maskedEmail: string }> {
    const user = await this.prisma.user.findUnique({ where: { id: userId } });
    if (!user) throw new ResourceNotFoundException('User');
    ...
    this.events.emit('audit.log', {
      ...
      action: AuditAction.PASSWORD_CHANGE,
      module: 'auth',
      description: 'Password change OTP requested',
      isSuccess: true,
    });
  ...
  async changePasswordWithOTP(
    ...
    // Validate OTP
    await this.tokenService.consumePasswordChangeOTP(userId, otp);
    ...
    this.events.emit('notification.send', {
      ...
      variables: {
        ...
        ipAddress: 'Not tracked', // Can add IP tracking if needed
        year: new Date().getFullYear(),
      },
    });
