...
import { ChangePasswordWithOtpDto } from './dto/change-password-otp.dto';
...
export class AuthController {
  ...
  @ApiOperation({ 
    ...
    description: 'Sends a 6-digit OTP to the user\'s registered email for password change verification'
  })
  ...
  @ApiOperation({ 
    ...
    description: 'Validates the OTP and changes the user password. All active sessions will be terminated.'
  })
  async changePasswordWithOTP(
    @CurrentUser() user: RequestUser,
    @Body() dto: ChangePasswordWithOtpDto,
  ): Promise<{ message: string; data: null }> {
    await this.authService.changePasswordWithOTP(user.id, dto.otp, dto.newPassword);
    return ok(null, 'Password changed successfully. Please log in again with your new password.');
  }
