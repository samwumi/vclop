...
export class TokenService {
  ...
  async issuePasswordChangeOTP(userId: string): Promise<string> {
    // Invalidate any existing OTPs for this user
    await this.prisma.token.updateMany({
      where: { userId, type: TokenType.PASSWORD_CHANGE_OTP, usedAt: null, revokedAt: null },
      data: { revokedAt: new Date() },
    });
    ...
    const expiresMinutes = this.config.get<number>('auth.otpExpiresIn') ?? 10;

    await this.prisma.token.create({
      data: {
        userId,
        type: TokenType.PASSWORD_CHANGE_OTP,
        token: otp,
        hashedToken: hashed,
        expiresAt: dayjs().add(expiresMinutes, 'minute').toDate(),
      },
  ...
  async consumePasswordChangeOTP(userId: string, otp: string): Promise<void> {
    const hashed = sha256(otp);
    ...
    if (!stored) throw new TokenInvalidException('Invalid OTP');
    if (stored.usedAt) throw new TokenInvalidException('OTP has already been used');
    if (stored.revokedAt) throw new TokenInvalidException('OTP has been revoked');
    if (stored.expiresAt < new Date()) throw new TokenExpiredException('OTP has expired');
